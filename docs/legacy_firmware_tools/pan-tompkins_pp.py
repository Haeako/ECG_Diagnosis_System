...
try:
    import scipy.signal.windows as sig_windows
except ImportError:
    sig_windows = sig
...
def load_ecg_csv(files, signal_column):
    timestamps = []
    signal_values = []
    csv_peak_indices = []
    ...
    return np.asarray(timestamps), np.asarray(signal_values), np.asarray(csv_peak_indices, dtype=int)


def plot_detection(times, ecg, detected_indices, csv_peak_indices, output, signal_column, title):
    import matplotlib

    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    valid_detected = detected_indices[
        (detected_indices >= 0) & (detected_indices < len(ecg))
    ].astype(int)
    valid_csv = csv_peak_indices[
        (csv_peak_indices >= 0) & (csv_peak_indices < len(ecg))
    ].astype(int)

    fig, axes = plt.subplots(2, 1, sharex=True, figsize=(16, 7))
    axes[0].plot(times, ecg, linewidth=0.65, color="#1f77b4", label=signal_column)
    axes[0].scatter(times[valid_detected], ecg[valid_detected], s=22, color="#d62728",
                    label="Pan-Tompkins++ R")
    if len(valid_csv):
        axes[0].scatter(times[valid_csv], ecg[valid_csv], s=36, facecolors="none",
    ...
    if len(bpm):
        axes[1].plot(times[valid_detected][1:], bpm, marker="o", markersize=3,
                     linewidth=0.9, color="#9467bd")
    axes[1].set_ylabel("BPM from RR")
...
def main():
    parser = argparse.ArgumentParser(description="Run Pan-Tompkins++ R-peak detection on ECG CSV files.")
    parser.add_argument("paths", nargs="*", default=["pre_move_data"],
                        help="CSV file(s) or folder(s) containing ecg_*.csv files")
    parser.add_argument("--fs", type=int, default=360, help="Sampling frequency in Hz")
    parser.add_argument("--column", default="filtered", help="CSV signal column to detect on")
    parser.add_argument("-o", "--output", type=Path, default=Path("visualize"),
                        help="Output folder, or output image path when --combined is used")
    parser.add_argument("--title", default="Pan-Tompkins++ R-peak Detection",
                        help="Plot title")
    parser.add_argument("--combined", action="store_true",
                        help="Plot all input files into one image instead of one image per CSV file")
    ...
    if args.combined:
        times, ecg, csv_peak_indices = load_ecg_csv(files, args.column)
        detected_indices = Pan_Tompkins_Plus_Plus().rpeak_detection(ecg, args.fs).astype(int)
        duration_s = times[-1] - times[0] if len(times) > 1 else 0.0
        bpm_est = (len(detected_indices) * 60.0 / duration_s) if duration_s > 0 else 0.0
        output = args.output
        if output.suffix == "":
            output = output / "pan_tompkins_pp_result.png"

        plot_detection(times, ecg, detected_indices, csv_peak_indices, output, args.column, args.title)
        print(f"Samples: {len(ecg)}")
        print(f"CSV is_peak: {len(csv_peak_indices)}")
        print(f"Pan-Tompkins++ peaks: {len(detected_indices)}")
    ...
    total_detected = 0
    for file_path in files:
        times, ecg, csv_peak_indices = load_ecg_csv([file_path], args.column)
        detected_indices = Pan_Tompkins_Plus_Plus().rpeak_detection(ecg, args.fs).astype(int)
        output = output_dir / f"{file_path.stem}_pan_tompkins_pp.png"
        title = f"{args.title} - {file_path.name}"
        plot_detection(times, ecg, detected_indices, csv_peak_indices, output, args.column, title)

        total_samples += len(ecg)
        total_csv_peaks += len(csv_peak_indices)
        total_detected += len(detected_indices)
        duration_s = times[-1] - times[0] if len(times) > 1 else 0.0
        bpm_est = (len(detected_indices) * 60.0 / duration_s) if duration_s > 0 else 0.0
        print(
            f"{file_path.name}: samples={len(ecg)}, csv_peak={len(csv_peak_indices)}, "
            f"ptpp_peak={len(detected_indices)}, bpm_est={bpm_est:.1f}, saved={output}"
        )

    print(f"Total samples: {total_samples}")
    print(f"Total CSV is_peak: {total_csv_peaks}")
    print(f"Total Pan-Tompkins++ peaks: {total_detected}")
    print(f"Saved {len(files)} plot(s) in: {output_dir}")
